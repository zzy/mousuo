...
///
/// The height, text size, radius, and focus ring match the `Md` button, so
/// an input and a button line up in a row. For file inputs, the browser's
/// upload button is restyled as plain text without a border.
...
/// handlers) are forwarded to the `<input>`. A `class` among them is appended
/// to the component's classes. The input fills the width of its container,
/// so size it through the container or with a width class. Set
/// `aria-invalid="true"` to show the error border and focus ring.
///
/// ```ignore
/// view! {
